...
| `src/generated/prisma-sqlite/` | Generated SQLite Prisma client (gitignored; regenerated by `prebuild:pos`, bundled in Electron) |
...
- SQLite Prisma client at `src/generated/prisma-sqlite/` is **not** committed — `src/generated/` is
  ...
  `npm run prisma:generate:sqlite` before `npm run dev:pos` or the first test run.
