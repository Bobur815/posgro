...
export function UserList() {
  ...
  const columns = [
    ...
    { key: 'name', header: t('users.name'), render: (user: UserListItem) => (i18n.language === 'uz' ? user.nameUz : user.nameRu) },
    { key: 'role', header: t('users.role'), render: (user: UserListItem) => (
      <RoleBadge $role={user.role}>{user.role === 'ADMIN' ? t('users.admin') : t('users.cashier')}</RoleBadge>
    )},
    { key: 'active', header: t('users.status'), render: (user: UserListItem) => (
      <Badge $active={user.active}>{user.active ? t('users.active') : t('users.inactive')}</Badge>
    )},
    ...
    <Container>
      ...
      {userToToggle && (
        <ConfirmDialog
          ...
          onConfirm={() => { handleToggleActive(userToToggle); setUserToToggle(null); }}
