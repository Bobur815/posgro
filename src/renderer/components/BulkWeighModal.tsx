...
export function BulkWeighModal({ product, onAddToCart, onPrintAndScan, onCancel }: BulkWeighModalProps) {
    ...
    <Overlay onClick={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <Modal>
        ...
        <WeightDisplay $status={status}>
          ...
          {weightKg > 0 && (
            <TotalPrice>{totalPrice.toLocaleString('ru-RU', { maximumFractionDigits: 0 })} сум</TotalPrice>
          )}
        ...
        <ButtonRow>
          <Button $variant="primary" onClick={handlePrintAndScan} disabled={status !== 'valid' || isCreating}>
            <Printer size={16} />
          ...
          <Button $variant="danger" onClick={onCancel} style={{ flex: '0 0 auto', width: '80px' }}>
            {t('common.cancel')}
          </Button>
        </ButtonRow>
      </Modal>
