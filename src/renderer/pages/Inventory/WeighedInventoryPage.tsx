...
const Badge = styled.span<{ $status: 'AVAILABLE' | 'SOLD' }>`
  ...
  background: ${({ $status, theme }) =>
    $status === 'AVAILABLE' ? theme.colors.success + '20' : theme.colors.textSecondary + '20'};
  color: ${({ $status, theme }) =>
...
export function WeighedInventoryPage() {
  ...
  useEffect(() => {
    window.electronAPI.products.getAll({ active: true }).then((prods) => {
      setProducts((prods as Product[]).filter((p) => p.productType === 'BULK_WEIGHTED'));
    }).catch(() => {});
  }, []);
  ...
  const soldToday = data.items.filter(
    (i) => i.status === 'SOLD' && i.soldAt && new Date(i.soldAt).toDateString() === today,
  ).length;
  ...
  return (
    <Container>
      ...
      <StatsRow>
        ...
        <StatCard>
          <StatValue>{Math.round(totalValue).toLocaleString('ru-RU')} {t('common.currency')}</StatValue>
          ...
          <tbody>
            {data.items.map((item) => (
              <tr key={item.id}>
                ...
                <Td style={{ fontWeight: 600 }}>{Math.round(item.totalPrice).toLocaleString('ru-RU')}</Td>
                <Td>
                  <Badge $status={item.status as 'AVAILABLE' | 'SOLD'}>
                    {item.status === 'AVAILABLE' ? t('inventory.available') : t('inventory.sold')}
                  </Badge>
                </Td>
