...
const StatusDot = styled.span<{ online: boolean }>`
  ...
  height: 10px;
  border-radius: 50%;
  background-color: ${({ online, theme }) => online ? theme.colors.success : theme.colors.error};
  margin-right: 8px;
`;
...
const Badge = styled.span<{ warn: boolean }>`
  ...
  font-size: 12px;
  font-weight: 600;
  background-color: ${({ warn, theme }) => warn ? theme.colors.error : theme.colors.success};
  color: #fff;
`;
...
export function TerminalStatus() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load terminal status');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <Container>
      <HeaderRow>
        ...
        <Button onClick={load} disabled={isLoading} style={{ marginRight: 24 }}>
      ...
      <Card>
        ...
        {statuses.length > 0 && (
          <Table>
            ...
            <tbody>
              {statuses.map((s) => {
                const isOnline = Date.now() - new Date(s.lastSyncAt).getTime() < OFFLINE_THRESHOLD_MS;
                return (
                  <tr key={s.terminalId}>
                    <Td><strong>{s.terminalId}</strong></Td>
                    <Td>
                    ...
                    <Td>
                      <Badge warn={s.unsyncedCount > 0}>{s.unsyncedCount}</Badge>
                    </Td>
                  </tr>
                );
              })}
